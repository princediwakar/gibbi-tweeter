...
export async function generateHighQualityTweets(options: AutoGenerationOptions): Promise<AutoGeneratedTweet[]> {
  ...
  console.log(`🤖 Starting auto-generation: target ${count} tweets, min quality ${minQualityScore}`);
  ...
  while (tweets.length < count && attempts < maxAttempts) {
    ...
    try {
      ...
      console.log(`📝 Attempt ${attempts}: Generating with ${persona}, hashtags: ${variation.includeHashtags}, trending: ${variation.useTrendingTopics}`);
      ...
      if (qualityScore.overall >= minQualityScore) {
        ...
        tweets.push(tweet);
        console.log(`✅ Quality tweet generated (${qualityScore.overall}/100, Grade: ${qualityScore.grade})`);
      } else {
        console.log(`❌ Low quality tweet rejected (${qualityScore.overall}/100, Grade: ${qualityScore.grade})`);
      }
  ...
  console.log(`🎯 Auto-generation complete: ${tweets.length}/${count} high-quality tweets generated in ${attempts} attempts`);
  return tweets;
}
...
export async function selectBestTweetsForPosting(tweets: AutoGeneratedTweet[], targetCount: number = 2): Promise<AutoGeneratedTweet[]> {
  ...
  console.log(`🏆 Selected ${selectedTweets.length} best tweets for posting:`);
